...
const mapPolicy = (policy: ActivePolicy | null): Policy | null => {
  if (!policy) return null;

  return {
    id: policy.id,
    plan: policy.plan,
    coverageAmount: Number(policy.max_payout || 0),
    premiumPaid: Number(policy.premium_amount || 0),
    endDate: policy.week_end,
  };
};

const mapClaims = (claims: ApiClaim[]): Claim[] =>
  claims.map(claim => {
    ...
    return {
      id: claim.id,
      type: claim.event_type || 'Disruption Claim',
      status,
      filedOn: formatDate(claim.created_at),
      payoutAmount: claim.payout_amount ? Number(claim.payout_amount) : null,
      created_at: claim.created_at,
    };
  });

const mapDisruptions = (disruptions: DisruptionEvent[]): Disruption[] =>
  disruptions.map(disruption => {
    const numericSeverity = Number(disruption.severity || 0);
    const severity: Disruption['severity'] =
      numericSeverity >= 0.7 ? 'high' : numericSeverity >= 0.4 ? 'medium' : 'low';

    return {
      id: disruption.id,
      type: disruption.event_type.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()),
      description: `${disruption.zone} zone impacted with severity ${(numericSeverity * 100).toFixed(0)}%`,
      city: disruption.city,
      severity,
      triggered_at: disruption.triggered_at,
    };
  });
...
export default function HomeScreen() {
  ...
  const loadData = async () => {
    try {
      ...
      const currentWorker = profileResponse.data.worker;
      const [policyResponse, claimsResponse, disruptionsResponse] = await Promise.all([
        getActivePolicy(),
        getClaims(),
      ...
      const storedPartner = stored ? JSON.parse(stored) : null;

      setPartner({
        ...currentWorker,
        memberSince: storedPartner?.memberSince || currentWorker.created_at,
      });
      setPolicy(mapPolicy(policyResponse.data));
      setClaims(mapClaims(claimsResponse.data.claims));
      setDisruptions(mapDisruptions(disruptionsResponse.data.disruptions));
    } catch (loadError) {
      const stored = await AsyncStorage.getItem(STORAGE_KEYS.PARTNER_DATA);
      if (stored) {
        const storedPartner = JSON.parse(stored) as Partner;
